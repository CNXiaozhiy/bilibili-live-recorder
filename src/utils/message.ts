...
interface CommandStep<U extends UserBase> {
  prompt?: string;
  validator?: (input: string, user: U) => boolean;
  required?: boolean;
  default?: any;
}
...
interface CommandConfig<U extends UserBase> {
  steps?: CommandStep<U>[];
  validator?: (params: string[], user: U) => string | void;
}
...
export default class CommandHandler<T extends string, U extends UserBase = UserBase> {
  ...
  async handleMessage(user: U, raw: string): Promise<Messages | null> {
    ...
    if (!command.config.steps) {
      try {
        const result = await command.handler(user, params);
        return result;
      } catch (e) {
    ...
    return this.getNextPrompt(command.config, validParams.length);
  }
  ...
  private async processRequiredStep(
    ...
    // 带用户上下文的参数验证
    if (stepConfig.validator && !stepConfig.validator(input, user)) {
      return stepConfig.prompt ? `${stepConfig.prompt}（输入无效）` : "输入无效";
    }
    ...
    if (newStep < state.requiredSteps) {
      ...
      return this.getNextPrompt(command.config, newStep);
    }
    ...
    return this.getOptionalPrompt(command.config, newState.optionalSteps[0]);
  }
  ...
  private async processOptionalStep(
    ...
    for (const [i, inp] of inputs.entries()) {
      ...
      // 带用户上下文的验证
      if (stepConfig.validator && !stepConfig.validator(inp, user)) {
        return stepConfig.prompt ? `${stepConfig.prompt}（输入无效）` : "输入无效";
      }
      ...
      if (remaining.some((s) => s.toLowerCase() === "q")) {
        shouldBreak = true;
        break;
      }
    }
    ...
    // 处理提前终止
    if (shouldBreak || inputs.some((s) => s.toLowerCase() === "q")) {
    ...
    return this.getOptionalPrompt(command.config, state.optionalSteps[currentIndex]);
  }
  ...
  private getNextPrompt(config: CommandConfig<U>, stepIndex: number): string {
    const step = config.steps?.[stepIndex];
    return step?.prompt || `请输入第 ${stepIndex + 1} 个参数：`;
  }

  private getOptionalPrompt(config: CommandConfig<U>, stepIndex: number): string {
    const requiredCount = config.steps?.filter((s) => s.required !== false).length || 0;
    const step = config.steps?.[stepIndex];
    return step?.prompt || `请输入可选参数（${stepIndex + 1 - requiredCount}）：`;
  }
  ...
  private async executeHandler(
    ...
    try {
      const validation = command.config.validator?.(params, user);
